...
open class QRCode: NSObject, AVCaptureMetadataOutputObjectsDelegate {
    ...
    func createPath(_ points: NSArray) -> UIBezierPath {
        ...
        guard let point = CGPoint(dictionaryRepresentation: (points[0] as! CFDictionary)) else {
            path.close()
            return path
        }
        ...
        while index < points.count {
            ...
            guard let newPoint = CGPoint(dictionaryRepresentation: (points[index] as! CFDictionary)) else {
                continue
            }
